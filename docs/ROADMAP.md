...
- **Peça 1a — Modelo genérico de tipos de resposta (schema) ✅ concluída (2026-07-23).** Ver detalhes na seção Progresso acima. Branch `worktree-modelo-generico-tipos-resposta`, não mesclada ainda. Design: `docs/superpowers/specs/2026-07-22-modelo-generico-tipos-resposta-design.md`, plano: `docs/superpowers/plans/2026-07-22-modelo-generico-tipos-resposta-1a-schema.md`.
- **Peça 1b — Adaptar a camada de app ao schema novo ⏳ próxima.** Server Actions (`actions.ts`), os forms de classificação/medição, o painel de lote, e as queries de página precisam trocar `classificacao`/`paint_measurements`/`save_paint_measurement`/`apply_classificacao_batch` (removidos pela Peça 1a) por `opcao_id`/`medicoes`/`save_medicao`/`apply_opcoes_batch`. **Sem redesign visual aqui** — mesma UI de hoje, só trocando o que ela lê/escreve no banco.
- **Peça 2 — Re-seed do checklist real (360 itens/13 categorias).** Substitui o seed atual (320 itens/12 grupos) pelo conteúdo de `checklist_inspecta_v7.md`, usando o modelo genérico da Peça 1a.
...
**Prompt (Peça 1b):**
> A Peça 1a (schema genérico de tipos de resposta) está pronta na branch `worktree-modelo-generico-tipos-resposta`, ainda não mesclada. Vamos desenhar a Peça 1b: adaptar `actions.ts`, os forms de item, o painel de lote e as queries de página pro schema novo, mantendo a UI de hoje sem mudança visual. Use `superpowers:brainstorming`.
