...
Todo o bloco 0 (housekeeping) está concluído — ver seção Progresso acima. `main` tem: 36 migrations (schema genérico de tipos de resposta — `conjuntos_opcao`/`opcoes`/`medicoes`, substituindo `classificacao`/`paint_measurements`), RLS nas 12 tabelas + policy de Storage, checklist seedado (320 itens/12 grupos, 101 deles com `grupo_replicacao` curado), código de app da Fase 1a (login + dados básicos), Fase 1 (navegação do checklist, PR #3 mesclada), validade da inspeção (PR #2 mesclada), Fase 2 (preenchimento de item, PR #4 mesclada, já adaptada ao schema genérico pela Peça 1b), Fase 2.5 (aplicar aos demais, mesclada e enviada, idem), Fase 2.6 (rename Check Auto, mesclada) e Fase 2.7 (design system aplicado a todas as telas, mesclada e enviada — porém **rejeitada pelo usuário depois de ver rodando**, ver Fase 2.8). `npm test` limpo em `main` (87/87), `tsc` limpo, docs sincronizados. Diretório de trabalho é `~/Desktop/bild app` (KINGSTON está corrompido/deprecated — nunca usar).

**Fase 2.8 (redesign do checklist) em andamento:** Peça 1a (schema genérico) e Peça 1b (camada de app adaptada ao schema novo) concluídas, revisadas e mescladas em `main` (2026-07-23) — ver detalhes na seção Progresso acima. **Próxima unidade de trabalho real: Peça 2** — re-seed do checklist real (360 itens/13 categorias de `checklist_inspecta_v7.md`), usando o modelo genérico agora disponível (incluindo, pela primeira vez, itens `tipo='texto'`/`tipo='data'`, que a Peça 1b deixou de propósito sem UI porque não havia item real desse tipo ainda). Depois: Peça 3 (o redesign visual/dashboard). Fase 3 (autosave, RF-25–27) fica depois dessas peças — a rejeição da Fase 2.7 interpôs esse trabalho antes dela.
...
- **Peça 2 — Re-seed do checklist real (360 itens/13 categorias) ⏳ próxima.** Substitui o seed atual (320 itens/12 grupos) pelo conteúdo de `checklist_inspecta_v7.md`, usando o modelo genérico das Peças 1a/1b — inclui, pela primeira vez, itens `tipo='texto'`/`tipo='data'` (a Peça 1b deixou esses 2 tipos sem UI de propósito, por não haver item real ainda).
- **Peça 3 — Redesign visual/dashboard.** Sidebar de navegação, tabela por subseção com edição inline, tabs de dados do veículo — no estilo da referência, mas com a paleta verde Check Auto (sem roxo) e rotas normais do Next.js (não SPA). Retoma o brainstorming pausado quando este chegar (várias decisões já fechadas: ver seção Progresso).

**Prompt (Peça 2):**
> As Peças 1a e 1b (schema genérico + camada de app) estão prontas e mescladas em `main`. Vamos desenhar a Peça 2: re-seed do checklist real (360 itens/13 categorias de `checklist_inspecta_v7.md`), usando o modelo genérico — incluindo os tipos `texto`/`data` que ainda não têm UI. Use `superpowers:brainstorming`.
