...
insert into public.checklist_item_templates (group_id, nome, tipo)
  select id, 'Para-choque dianteiro', 'escolha' from public.checklist_group_templates where ordem = 950;
do $$
...
begin
  begin
    insert into public.checklist_item_templates (group_id, nome, tipo, qtd_pontos_medicao)
      select id, 'Espessura porta dianteira', 'medicao', null from public.checklist_group_templates where ordem = 950;
...
insert into public.checklist_item_templates (group_id, nome, tipo, qtd_pontos_medicao, aplica_stand)
  select id, 'Espessura porta dianteira', 'medicao', 4, true from public.checklist_group_templates where ordem = 950;
